...
# @return new_genotype
generateGenotype <- function(genotypes, dis_snp_pos1, dis_snp_pos2, loop_count) {
     ...
     x_spec1 <- as.numeric(genotypes[samples_index[1], dis_snp_pos1]
                          + genotypes[samples_index[2], dis_snp_pos1])
     
     x_spec2 <- as.numeric(genotypes[samples_index[1], dis_snp_pos2]
                           + genotypes[samples_index[2], dis_snp_pos2])
     ...
     exp_value <- exp(alpha + (gene_effect * x_spec1) + (gene_effect * x_spec2))
...
if (length(replicate_range) == 2 && replicate_range[1] < replicate_range[2]) {
     ...
     dis_snp_pos1 <- match(dis_snp1, snpsName)
     dis_snp_pos2 <- match(dis_snp2, snpsName)
     ...
     for (i in replicate_range[1]:replicate_range[2]) {
          ...
          while (case_index < number_of_case || 
               ...
               new_genotype <- generateGenotype(genotypes, dis_snp_pos1, dis_snp_pos2, loop_count)
          ...
          outfile_path <- "result/gen-snps/case-control-replicated-multiple-snps/0.2/"
